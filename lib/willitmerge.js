...
willitmerge.options = {
  ...
  repo: '',
  ignore: [],
  tmp: path.join(process.cwd(), '_tmp') + path.sep,
  remote: '',
  prepend: 'willitmerge-',
  page: 0,
  perpage: 30,
    ...
    function validateGitStatus(next) {
      execSeries([
        ...
        if (/changed but not updated|changes to be committed/i.test(status)) {
          that.emit('error', new Error('Please commit or stash your changes before running willitmerge.'));
        }

        // get original branch
        var found = /# on branch (.*)/i.exec(status);
        if (!found) {
          that.emit('error', new Error('Could not determine the current branch. Is this a git repo?'));
          return false;
        }
        origBranch = found[1];
...
willitmerge.onEnd = function(issues) {
  ...
  if (path.dirname(this.options.tmp) === process.cwd()) {
    this.file.rmdir(this.options.tmp);
  }
};
