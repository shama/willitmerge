...
willitmerge.testIssue = function(iss, done) {
  ...
  var gitUrl = iss.head && iss.head.repo && iss.head.repo.git_url;

  // issue skipped or git url no longer exists
  if (_.indexOf(that.options.ignore, Number(iss.number)) !== -1 || gitUrl === null) {
    iss.willitmerge = {
      status: 'skipped',
      msg: '',
      impact: 0
...
willitmerge.reportIssues = function(issues) {
  var that = this;
  var verb = that.options.rebase ? 'rebase' : 'merge';
  ...
  issues.forEach(function(iss) {
    if (iss.willitmerge.status === 'skipped') {
      ...
      that.log.write('Issue #' + iss.number + ', ' + iss.html_url + ', will it ' + verb + '? ');
